...
void calcolaFrequenze(int a[],int b[],int c[],int *cont,int *contL){
    int freq;
...
int main(){
  int numeri[DIM], ordinati[DIM], freq[DIM];
  int cont = 0, contF = 0, contL = 1;
  int max = -1, min;
  float media;
  ...
  calcolaFrequenze(numeri,ordinati,freq,&cont,&contL);
  printf("Massimo: %d\nMinimo: %d\nMedia: %f\n",max,min,media);
  for(int i = 0; i < contL; i++){
    printf("%d-%d\n", ordinati[i],freq[i]);
  }
