...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        puts("Inserire il nome_vino utente ('fine' per terminare):");
        if (fgets(nome_vino, sizeof(nome_vino), stdin) == NULL) {
            perror("fgets nome_vino");
            exit(EXIT_FAILURE);
        }
        // Esco se l'utente inserisce 'fine'
        if (strcmp(nome_vino, "fine\n") == 0) {
            printf("Ricevuto 'fine' ... terminazione\n");
            break;
        }

        puts("Inserire il anno ('fine' per terminare):");
