...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        puts("Inserire il localita utente ('fine' per terminare):");
        if (fgets(localita, sizeof(localita), stdin) == NULL) {
            perror("fgets localita");
            exit(EXIT_FAILURE);
        }
        ...
        puts("Inserire il N_localita ('fine' per terminare):");
        if (fgets(N_localita, sizeof(N_localita), stdin) == NULL) {
            perror("fgets N_localita");
            exit(EXIT_FAILURE);
        }
