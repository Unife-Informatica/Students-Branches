...
public class BollettinoNeve {
    public static void main(String[] args) {
        ...
        try {
            ...
            while (true) {
                System.out.println("Inserisci localita utente ('fine' per terminare):");
                String localita = userIn.readLine();
                if (localita.equals("fine"))
                    break;
                
                System.out.println("Inserisci nome progetto da ricercare ('fine' per terminare):");
                String N_localita = userIn.readLine();
                if (N_localita.equals("fine"))
                    break;
                
                networkOut.write(localita);
                networkOut.newLine();
                networkOut.write(N_localita);
                networkOut.newLine();
                networkOut.flush();
