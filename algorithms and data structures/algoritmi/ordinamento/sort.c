...
void init(struct configuration *config) {
    ...
    // ESERCIZIO: prova a scrivere un ciclo che chiede all'utente 
    // quanti algoritmi vuole, e inserisca i loro nomi tra una
    // lista di proposte.
    int n_algorithms = 5;
    char *algos[] = {"INSERTION", "MERGE", "HYBRIDMERGE", "QUICK", "MOTQUICK"};
        // "TAILQUICK", "HEAPSORT", "INTROSORT"};
...
/**
 * Vedi `intro sort`.
 * Questa è la sua sottoroutine principale.
 */
 ...
 * Qui, si calcola la soglia di chiamate ricorsive del `quick sort`
 * che verrà chiamato internamente.
 * Vedi `_intro_sort(int *, int, int, int)`.
 */
