...
try:
    assert w_result.shape == (ncols,), f"Errore: La dimensione preallocata è {w_result.shape}, ma era attesa ({ncols},)."
    
    assert A[0, 0] == 1.0, "Errore: La matrice originale A è stata modificata. Utilizzare un metodo esplicito per disaccoppiare la memoria."
    
    expected_stable = np.exp(x_val) * (np.e - 1.0)
    assert np.isclose(f_x_stable, expected_stable, rtol=1e-14), "Errore: La valutazione non è ottimizzata. Implementare la forma fattorizzata."
    print("Script 3: Validazione superata. Preallocazione, disaccoppiamento memoria e formulazione algebrica corretti.\n")
