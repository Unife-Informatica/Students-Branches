...
try:
    assert np.allclose(domain[1:-1, 1:-1], 0.5), "Errore: La sottomatrice interna di 'domain' non è stata alterata."
    assert domain[0, 0] == 1.0, "Errore: Le condizioni al contorno (Dirichlet) sono state corrotte."
    
    # Validazione del corretto broadcasting column-wise
    expected_col_0 = np.array([0.9, 0.4, 0.4, 0.4, 0.9])
    assert np.allclose(domain_corrected[:, 0], expected_col_0), "Errore: Il vettore di bias è stato sottratto per righe invece che per colonne."
    print("Script 2: Validazione superata. Viste e Broadcasting implementati con successo.\n")
