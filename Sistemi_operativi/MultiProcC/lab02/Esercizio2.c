...
void sigusr1_handler(int sig){
  counter++;
  printf("Child %d: Incremento il contatore\n", child_index);
  printf("Nuovo valore contatore child %d: %d\n", child_index, counter);
}

void sigusr2_handler(int sig){
  counter--;
  printf("Child %d: Decremento il contatore\n", child_index);
  printf("Nuovo valore contatore child %d: %d\n", child_index, counter);
}
...
int main(int argc, char *argv[]){
  ...
  for(int i = 0; i < nChildren; i++){
    ...
    if(pid == 0){
      ...
      if(sigation(SIGUSR1, &sigusr1, NULL) == -1){
      ...
      while(1){
        /* Attraverso pause() il processo dorme in attesa di
           ricevere un segnale.*
           Una volta terminata l'esecuzione dell'handler corrispondente
           il processo tornerà in attesa di ricevere il prossimo segnale da gestire */
        pause();
      }
    }else if(pid < 0){
  ...
  // Codice Parent
  // Installo gestore segnale SIGINT
  ...
  while(!flagTerminato){
    // Scelgo casualmente segnale SIGUSR1/SIGUSR2 e child a cui mandarlo
