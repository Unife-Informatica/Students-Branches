...
public class Main {
    public static void main(String[] args) {
        ...
        List<Transazione> transazioniNovembre = transazioni.stream()
            .filter(t -> t.getData().getMonth() == Month.NOVEMBER)
            .collect(Collectors.toList());
        System.out.println(transazioniNovembre);

        // 2 - Calcolare l'importo totale dei crediti e dei debiti separatamente
        double crediti = transazioni.stream()
            .filter(t -> t.getTipo().equals("Credito"))
            .map(t -> t.getImporto())
            .reduce(0.0, (a, b) -> a + b);

        double debiti = transazioni.stream()
            .filter(t -> t.getTipo().equals("Debito"))
            .map(t -> t.getImporto())
            .reduce(0.0, (a, b) -> a + b);

        System.out.println(crediti);
        System.out.println(debiti);
